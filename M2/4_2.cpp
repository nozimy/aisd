...
private:
    ...
    AVLNodePtr addInternal(AVLNodePtr node, const T &data)
    {
        if (!node)
    ...
    AVLNodePtr removeInternal(AVLNodePtr node, const T &data)
    {
        if (!node)
            return nullptr;
        if (node->data < data)
        ...
        {
            AVLNodePtr left = node->left;
            AVLNodePtr right = node->right;
    ...
    AVLNodePtr removeMin(AVLNodePtr node)
    {
        if (!node->left)
            return node->right;
